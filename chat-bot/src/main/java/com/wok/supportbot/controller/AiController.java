...
public class AiController {
    ...
    /**
     * 同步调用 AI 智能客服应用（带商品卡片推荐）
     *
     * @param message
     * @param chatId
     * @return
     */
    @GetMapping("/assistant_app/chat/with-products")
    public Result<ChatResponseDTO> doChatWithProducts(String message, String chatId) {
        ChatResponseDTO response = assistantApp.doChatWithProduct(message, chatId);
        return Result.success(response);
    }

    /**
     * 同步调用 AI 智能客服应用（带工具调用：天气/新闻 + 商品推荐）
     *
     * @param message
     * @param chatId
     * @return
     */
    @GetMapping("/assistant_app/chat/with-tools")
    public Result<ChatResponseDTO> doChatWithTools(String message, String chatId) {
        ChatResponseDTO response = assistantApp.doChatWithTools(message, chatId);
        return Result.success(response);
    }
